...
var config = require('../lib/config').section('cache', {refreshRate: 20000, concurrency: 5});
...
var Cache = function(name, options){
  ...
  self.updateCallback = options.getUpdate;//updateCallback;
  self.update();
};
    ...
    if(!record){
      ...
      self.updateCallback(id, function(err, record){
        ...
        self.store.insert(record, function(err, record){
          if((!err) &&(!!self.options.onAdded)){
            self.options.onAdded(record);
          }
          callback(err, record);
        });
      });
...
Cache.prototype.getUpdatedRecordById = function(id, callback){
  var self = this;
  self.store.get(id, function(err, record){
    if(err){
      logger.error(err);
    ...
    if(!record){
      self.updateCallback(id, function(err, record){
        ...
        record._id = id;
        logger.debug('Record:', record._id, 'inserted');
        self.store.insert(record, function(err, record){
          if((!err) &&(!!self.options.onUpdated)){
            self.options.onUpdated(record);
          }
      ...
      self.updateCallback(record._id, function(err, update){
        ...
        if(equal(record, update, ['_updated', '_created', '_id', 'id'])){
          ...
          logger.debug('Record:', record._id, 'updated');
          self.store.update(record._id, update, function(err, record){
            if((!err) &&(!!self.options.onUpdated)){
              self.options.onUpdated(record);
            }
            callback(null, record);
          ...
          logger.debug('Record:', record._id, 'no update received');
          callback(null, record);
        }
      });
    }
...
Cache.prototype.update = function(){
  ...
  if(!!self.options.getListing){
    logger.debug('Getting listing');
    self.options.getListing(function(err, records){
      if(err){
        logger.error(err);
        return done();
      }
      logger.debug('Got listing with '+records.length+' records');
      async.eachLimit(records, config.concurrency, function(record, next){
        self.getUpdatedRecordById(record._id, function(err, record){
          next();
        });
      }, done);
    });
