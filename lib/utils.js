...
var defaultHostName = 'u360app.com';
...
var forcehost = false;
if(env!=='production'){
  forcehost = require('../lib/config').section('web').host||false;
  if(forcehost){
    console.log('Host forced to:', forcehost);
  }
}

module.exports.getHostName = function(req){
  var host = forcehost || req.headers.host || '';
  var hostName = ((host.match(/([a-z0-9\_]+)(\:\d+)/i)||[])[1]||host).replace(/^www\./i, '');
  if(req.query._site){
    req.session.host = req.query._site;
  }
  if((req.session||{}).host){
    hostName = req.session.host;
  }
  var hostParts = hostName.split('.');
  var reSiteExt = /(org|net|com|us|edu)/i;
  var i = 1, l = hostParts.length, hostFound = false;
  while((i<l) && (!hostFound)){
    if((hostParts[i]||'').match(reSiteExt)){
      hostName = hostParts.slice(0, i+1).join('.');
      hostFound = true;
    }
    i++;
  }
