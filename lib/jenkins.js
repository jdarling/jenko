//System logs are at /log/rss
//Polling log: /job/<jobName>/<buildNumber>/pollingLog/pollingLog
// /job/<jobName>/scmPollLog/
...
var defaultOptions = {
  ...
  endpoints: {
    sysLog: {
      url: '{{config.host}}/log/rss',
      method: 'GET'
    },
    buildPollLog: {
      url: '{{config.host}}/job/{{args.name}}/{{args.build}}/pollingLog/pollingLog',
      method: 'GET'
    },
    newJob: {
...
Jenkins.prototype.makeRawCall = function(method, url, args, body, callback){
  ...
  request(opts, function(error, response, body) {
      if ( error || response.statusCode !== 200 ) {
        var err = error || {error: 'Empty body'};
        err.url = url;
        err.method = method;
        err.args = args;
        logger.log(err);
      ...
      if(!body){
        logger.log('No body received: ', opts.url);
      }
